...
- 桌面问答白名单走原生 Conversation + AgentRun 链路（与 Web 同构）：`GET /api/agent/default` → 幂等 `POST /api/chat/thread`（body 必填 `agent_id` + 受限格式 `thread_id`）→ `POST /api/agent/runs`（必填 `query/agent_slug/thread_id/meta.request_id`，`client=rice-endosperm-desktop`）→ `GET /api/agent/runs/{run_id}/result`。改这些路由的 schema 时两端同步；`apisix.yaml` 变更必须重建容器才生效。
...
- 企业批量开户走一次性激活凭证：管理员经 `POST /api/admin/onboarding/invitations` 单事务建户并签发激活码（只存哈希、24h 有效、单次消费、可撤销重签）；用户桌面端经 `POST /api/auth/onboarding/exchange` 兑换纯会话对，不产生静态 Key。注意 TIMESTAMPTZ 列读回为 aware，与 naive 比较需先归一（用 coerce_any_to_utc_datetime）。
...
- **TIMESTAMPTZ 列的时间比较必须用 aware datetime**：新表（device_sessions/onboarding_activations/usage_ledger 等）为 TIMESTAMPTZ，读回值带时区；与 `utc_now_naive()` 直接比较会抛 TypeError。写入与比较统一用 `utc_now()`（aware）；仅旧表（users/cli_auth_sessions 等 TIMESTAMP 列）继续用 naive。**ORM↔DB 列漂移自检**：给模型新增/改名任何列必须同步写迁移——曾因 `api_keys.tenant_id` 只声明未迁移导致建 Key、设备码登录、删用户三条链路同时 500；提交前用 `information_schema.columns` 对照 SQLAlchemy metadata 全量比对即可发现。
