...
1. 对知识问答优先调用 `query_knowledge_scope`，不要自行选择一个知识库来代替全范围检索。
2. 需要确认当前会话有哪些知识库可用时调用 `list_kbs`。
3. 只有在用户明确指定单库，或需要继续深挖某一来源时，才使用 `query_kb`。
4. 如果检索片段不足以回答，使用返回的 `file_id` 调用 `open_kb_document` 查看上下文。
...
- 关键科研论断必须引用 `query_knowledge_scope` 返回的 `evidence_id`；候选与冲突证据必须保留其状态。
