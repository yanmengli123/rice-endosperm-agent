...
- 新增托管知识图谱导入：Milvus 知识库可上传节点 CSV、关系 CSV 和可选 Cypher 说明文件，完成预检、基于注册标识的实体归一化、大小写/语义类型冲突人工裁决、后台导入、历史查询和来源感知回滚。PostgreSQL 作为实体、三元组、关系证据与逐行 provenance 的规范数据源，并在同一事务写入 Neo4j/Milvus Outbox；Neo4j 与 Milvus 只作为可重建投影，任务仅在三端按实体与三元组 ID 精确对账后成功。Cypher 文件只做语句和写关键字审计，绝不执行。修复 Milvus 删除旧投影后未及时 flush 导致同批次重投影漏写实体的问题，并补充真实水稻图谱数据端到端验收。
